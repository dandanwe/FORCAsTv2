      ...
      real jjpar(nmpmpo), jjorganion, jjh2oorg,diffs(nmpmpo), gas1(nmpmpo), gas2(nmpmpo)
...
!shc code below has been changed significantly from the original 
      ...
      if(is_run_mpmpo ) then
         call MAIN_MPMPO(jjcpt, jjcpart, jjtemp, jjrh, jjlwc, jjhplus, &
              jjpar, jjorganion, jjh2oorg, LOGDEV, LAYER)
...
! 86: mpmpo_gas.out
      ...
      if ((mod(tstamp,dble(1800.0))).eq.0) then 
        ...
        if ( is_run_mpmpo ) then
           write(85,987) tstamp,zlev,(jjpar(I),I=1,nmpmpo),jjcpt(6),totsoa,jjorganion, jjh2oorg !shc added jjorganion, jjh2oorg
...
987   format(f10.1,x,f10.4,2x,21(E13.4E3,1x))  !shc
